...
import { handleGetPaginatedWorkflowRunTimelineRequest } from '../../../presentation/http/handlers/handle-get-paginated-workflow-run-timeline-request.js';
...
export function createRouter(
  ...
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (method === 'GET' && path === '/workflow-runs/evidence-pack') {
        await handleGetWorkflowRunEvidencePackRequest(
          request,
          response,
          logger,
          correlationId,
      ...
      if (method === 'GET' && path === '/workflow-runs/timeline/page') {
        await handleGetPaginatedWorkflowRunTimelineRequest(
          request,
          response,
          logger,
          correlationId,
          dependencies.getPaginatedWorkflowRunTimelineUseCase,
        );
        return;
      }

      if (method === 'GET' && path === '/workflow-runs/timeline/search') {
        await handleSearchWorkflowRunTimelineRequest(
          request,
          response,
          logger,
          correlationId,
          dependencies.searchWorkflowRunTimelineUseCase,
        );
        return;
      }

      if (method === 'GET' && path === '/workflow-runs/realtime/events') {
        await handleWorkflowRunRealtimeEventsRequest(
          request,
          response,
          logger,
          correlationId,
          dependencies.workflowRunReadRepository,
          dependencies.realtimeEventHub,
        );
        return;
      }

      if (method === 'GET' && path === '/workflow-runs/realtime/diagnostics') {
        await handleGetWorkflowRunRealtimeDiagnosticsRequest(
          request,
          response,
          logger,
          correlationId,
      ...
      if (method === 'GET' && path === '/workflow-runs/realtime/staleness') {
        await handleGetWorkflowRunRealtimeStalenessRequest(
          request,
          response,
          logger,
          correlationId,
      ...
      if (method === 'GET' && path === '/simulation/scenarios/detail') {
        await handleGetSimulationScenarioDetailRequest(
          request,
          response,
          logger,
          correlationId,
